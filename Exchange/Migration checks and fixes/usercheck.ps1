...
if ($users -eq $null)
{
	#change to get all mail enabed object to include people that have migrated to exchanbge online already
	
$Pusers = get-mailbox -resultsize unlimited
$rusers = get-remotemailbox -resultsize unlimited
...
foreach ($p in $users)
{
	...
	$upnh = ($p.userprincipalname -split "@")[0]
    if ($upnh -eq $p.samaccountname)
    {
