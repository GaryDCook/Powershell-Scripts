...
function Get-DomainInfo($DomainName)

    {
        ...
        $DomainInfo = Get-ADDomain $DomainName -Credential $cred
        #$DomainInfo = Get-ADDomain "hq.globalcashaccess.us" -Credential $cred
        ...
        $DCListFiltered = Get-ADDomainController -Credential $cred -Server $domain -Filter { operatingSystem -like "Windows Server 2008 R2*" -or operatingSystem -like "Windows Server 2012*" -or operatingSystem -like "Windows Server Technical Preview"  } | Select * -ExpandProperty Name
        ...
        if ( $DCListFilteredIndex -eq 1 )
            
            {
                ...
                $pwdGPO = Get-ADDefaultDomainPasswordPolicy -Credential $cred -Server $DCListFiltered
                ...
                if ( $dfl -like "Windows2008Domain" -or $dfl -like "Windows2008R2Domain" -or $dfl -like "Windows2012Domain" -or $dfl -like "Windows2012R2Domain" )

                    {
                        ...
                        $FGPPNo = (Get-ADFineGrainedPasswordPolicy -Credential $cred -Server $DCListFiltered -Filter * | Measure-Object).Count
                ...
                $builtinAdmin = Get-ADuser -Credential $cred -Identity $domainSID-500 -Server $DCListFiltered -Properties Name, LastLogonDate, PasswordLastSet, PasswordNeverExpires, whenCreated, Enabled



                # Get total number of Domain Administrator group members
                $domainAdminsNo = (Get-ADGroup -Credential $cred -Identity $domainSID-512 -Server $DCListFiltered | Get-ADGroupMember -Credential $cred -Recursive | Measure-Object).Count
        ...
        # if there are more than one Windows Server 2008R2 Domain Controllers
        ...
        
            {
                ...
                $pwdGPO = Get-ADDefaultDomainPasswordPolicy -Credential $cred -Server $DCListFiltered[0]
                ...
                if ( $dfl -like "Windows2008Domain" -or $dfl -like "Windows2008R2Domain" -or $dfl -like "Windows2012Domain" -or $dfl -like "Windows2012R2Domain" )
                
                    {
                        ...
                        $FGPPNo = (Get-ADFineGrainedPasswordPolicy -Credential $cred -Server $DCListFiltered[0] -Filter * | Measure-Object).Count
                ...
                # Get information about built-in domain Administrator account
                $builtinAdmin = Get-ADuser -Credential $cred -Identity $domainSID-500 -Server $DCListFiltered[0] -Properties Name, LastLogonDate, PasswordLastSet, PasswordNeverExpires, whenCreated, Enabled



                # Get total number of Domain Administrators group members
                $domainAdminsNo = (Get-ADGroup -Credential $cred -Identity $domainSID-512 -Server $DCListFiltered[0] | Get-ADGroupMember -Credential $cred -Recursive | Measure-Object).Count
        ...
        $ou_objectsNo = (Get-ADOrganizationalUnit -Credential $cred -Server $domain -Filter * | Measure-Object).Count

        $cmp_objects = Get-ADComputer -Credential $cred -Server $domain -Filter * -Properties operatingSystem
        ...
        $frs = Get-ADObject -Credential $cred -Filter { distinguishedName -eq $FRSsysvol }
        $dfsr = Get-ADObject -Credential $cred -Filter { distinguishedName -eq $DFSRsysvol } 
        ...
        $orphaned = Get-ADObject -Credential $cred -Filter * -SearchBase "cn=LostAndFound,$($domainDN)" -SearchScope OneLevel | Measure-Object
        ...
        $lingConfRepl = Get-ADObject -Credential $cred -LDAPFilter "(cn=*\0ACNF:*)" -SearchBase $domainDN -SearchScope SubTree | Measure-Object
        ...
        $gpoDefaultDomain = Get-ADObject -Credential $cred -Server $domain -LDAPFilter "(&(objectClass=groupPolicyContainer)(cn={31B2F340-016D-11D2-945F-00C04FB984F9}))"
        $gpoDefaultDomainController = Get-ADObject -Credential $cred -Server $domain -LDAPFilter "(&(objectClass=groupPolicyContainer)(cn={6AC1786C-016F-11D2-945F-00C04fB984F9}))"
    ...
    if ( $args.Length -gt 0 )
    
        {
            ...
            $cred = $args[0]
            $ForestInfo=Get-ADForest $args[1] -Credential $cred
    ...
    $forestDomainSID = Get-ADDomain (Get-ADForest).Name -Credential $cred | Select domainSID
    ...
    if ( $ffl -like "Windows2008R2Forest" -or $ffl -like "Windows2012Forest" -or $ffl -like "Windows2012R2Forest" )
    
        {
            ...
            $ADRecBin=(Get-ADOptionalFeature -Credential $cred -Server $forest -Identity 766ddcd8-acd0-445e-f3b9-a7f9b6744f2a).EnabledScopes | Measure-Object
    ...
    $SchemaVersion = Get-ADObject -Credential $cred -Server $forest -Identity $SchemaPartition -Properties * | Select objectVersion
    ...
    $ExchangeSystemObjects = Get-ADObject -Credential $cred -Server $forest -LDAPFilter "(&(objectClass=container)(name=Microsoft Exchange System Objects))" -SearchBase $forestDN -Properties objectVersion
    $ExchangeSchemaVersion = Get-ADObject -Credential $cred -Server $forest -LDAPFilter "(&(objectClass=attributeSchema)(name=ms-Exch-Schema-Version-Pt))" -SearchBase $SchemaPartition -Properties rangeUpper
    ...
    if ($ExchangeSchemaVersion -ne $nul)
    
        {
            ...
            $ExchOrganization = (Get-ADObject -Credential $cred -Server $forest -Identity "cn=Microsoft Exchange,cn=Services,$configPartition" -Properties templateRoots).templateRoots
            $ExchOrgName = (Get-ADObject -Credential $cred -Server $forest -Identity $($ExchOrganization -Replace "cn=Addressing," , "") -Properties name).name
    ...
    $LyncSchemaVersion = Get-ADObject -Credential $cred -Server $forest -LDAPFilter "(&(objectClass=attributeSchema)(name=ms-RTC-SIP-SchemaVersion))" -SearchBase $SchemaPartition -Properties rangeUpper
    ...
    $tombstoneLifetime = (Get-ADobject -Credential $cred -Server $forest -Identity "cn=Directory Service,cn=Windows NT,cn=Services,$configPartition" -Properties tombstoneLifetime).tombstoneLifetime
    ...
    $ADTrusts = Get-ADObject -Credential $cred -Server $forest -Filter { objectClass -eq "trustedDomain" } -Properties CanonicalName,trustDirection
    ...
    $partitions = Get-ADObject -Credential $cred -Server $forest -Filter * -SearchBase $ForestInfo.PartitionsContainer -SearchScope OneLevel -Properties name,nCName,msDS-NC-Replica-Locations | Select name,nCName,msDS-NC-Replica-Locations | Sort-Object name
    ...
    $AllSites = Get-ADObject -Credential $cred -Server $forest -Filter { objectClass -eq "site" } -SearchBase $ConfigurationPart -Properties *
    ...
    foreach ( $Site in $AllSites )
        
        {
            ...
            $ServersInSite = Get-ADObject -Credential $cred -Server $forest -Filter { objectClass -eq "server" } -SearchBase $Site.distinguishedName -SearchScope Subtree -Properties Name | Select Name | Sort-Object Name
            ...
            
                {
                    ...
                    if ( $Server -ne $nul )
                    
                        {
                            ...
                            $dcDetails = Get-ADDomainController $Server.Name -Credential $cred
                            ...
                            $dcFRSinfo = Get-ADObject -Credential $cred -Filter { distinguishedName -eq $dcFRS } -Properties fRSRootPath
                            $dcDFSRinfo = Get-ADObject -Credential $cred -Filter { distinguishedName -eq $dcDFSR } -Properties msDFSR-RootPath, msDFSR-RootSizeInMb
    ...
    $siteLinks = Get-ADObject -Credential $cred -Server $forest -Filter { objectClass -eq "siteLink" } -SearchBase $ConfigurationPart -Properties name, cost, replInterval, siteList | Sort-Object replInterval
    ...
    
        {
            ...
            foreach ($linkList in $link.siteList)
            
                {
                    ...
                    $siteName = Get-ADObject -Credential $cred -Identity $linkList -Properties Name
    ...
    $schemaGroupID = ((Get-ADDomain($forest).name -Credential $cred).domainSID).value+"-518"
    $schemaAdminsNo = Get-ADGroup -Credential $cred -Server $forest -Identity $schemaGroupID | Get-ADGroupMember -Credential $cred -Recursive
    ...
    # Enterprise Admins
    $entGroupID = ((Get-ADDomain($forest).name -Credential $cred).domainSID).value+"-519"
    $enterpriseAdminsNo = Get-ADGroup -Credential $cred -Server $forest -Identity $entGroupID | Get-ADGroupMember -Credential $cred -Recursive
    ...
    $allDomains | Sort | %{ Get-DomainInfo -Credential $cred ($_) }
    #get-domaininfo("hq.globalcashaccess.us")
